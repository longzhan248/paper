...
//#define CancelStrImgaeName @"PUBLISH_CLOSE_IMG"
...
- (void)initUIsize
{
    ...
    if (_backgroundType == HyPopMenuViewBackgroundTypeDarkBlur || _backgroundType == HyPopMenuViewBackgroundTypeDarkTranslucent || _backgroundType == HyPopMenuViewBackgroundTypeGradient) {
        [bottomView setBackgroundColor:[UIColor colorWithRed:90.f / 225.f green:90.f / 225.f blue:90.f / 225.f alpha:0.6f]];
    }
    ...
    [disappearButton setBackgroundImage:[UIImage imageNamed:PUBLISH_CLOSE_IMG] forState:UIControlStateNormal];
    [disappearButton addTarget:self  action:@selector(closeMenu) forControlEvents:UIControlEventTouchUpInside];
    CGFloat CANCELw = 31;//28
...
- (CGRect)getFrameAtIndex:(NSUInteger)index;
{
    ...
    //150
    CGFloat buttonViewY = ((rowIndex + 1) * margin + rowIndex * buttonViewHeight) + (kH - toHeight) - 80;
...
- (void)selectedFunc:(PopMenuButton*)sender
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [CommonUtil registerNotice:@"skip_controller" object:idxStr];
    });
