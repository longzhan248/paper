...
- (void)registerAsDodgeViewForMLInputDodger
{
    if (![[MLInputDodger dodger] isRegisteredForDodgeView:self]) {
        NSLog(@"%f",self.originalYAsDodgeViewForMLInputDodger);
        if (self.originalYAsDodgeViewForMLInputDodger==0) {
            if (self.shiftHeightAsDodgeViewForMLInputDodger==81) {
                self.originalYAsDodgeViewForMLInputDodger = self.frame.origin.y-NAVIGATIONHEIGHT;
            }else{
